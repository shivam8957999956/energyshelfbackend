...
router.get("/questionRequested/", async (req, res) => {
  const username = req.query.user;
  const admin = req.query.admin;
  const catName = req.query.cat;
  const compName = req.query.comp;
  ...
  try {
    ...
    if (admin) {
      if (sortName) {
        ...
        posts = await Post.find({ admin, approved: { $eq: false } }).sort({
          createdAt: -1,
        });
    } else if (username) {
      if (sortName) {
        posts = await Post.find({ username, approved: { $eq: false } }).sort({
          createdAt: -1,
        });
        // console.log(sort);
      } else
      ...
      posts = await Post.find({ approved: { $eq: false } }).sort({
        createdAt: -1,
      });
    }
    res.status(200).json(posts);
